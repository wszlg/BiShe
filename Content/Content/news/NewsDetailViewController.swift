...
class NewsDetailViewController: UITableViewController, UITextFieldDelegate {
...
//            print("===\(erroe!)===")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 0 {
            let cell = tableView.dequeueReusableCell(withIdentifier: "NewsDetailCell1", for: indexPath) as! NewsDetailCell1
            cell.title.text = model["title"].stringValue
            cell.content.text = model["content"].stringValue
            cell.img.kf.setImage(with: ImageResource(downloadURL: URL(string: model["picurl"].stringValue)!))
            return cell
        } else {
